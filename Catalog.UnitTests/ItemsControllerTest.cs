...
namespace Catalog.UnitTests
{
    public class ItemsControllerTests
    {
        ...
        public async Task GetItemAsync_WithExistingItem_ReturnsExpectedItem()
        {
            ...
            // better way using FluentAssertions library, compares properties of Dto with that of expectedItems
            result.Value.Should().BeEquivalentTo(expectedItem);
        }

        [Fact]
        public async Task GetItemsAsync_WithExistingItems_ReturnsLAllItems()
        {
            // Arrange
            ...
            for (int i = 0; i < 5; i++)
            {
                expectedItemList.Add(CreateRandomItem());
            };

            _repositoryStub.Setup(repo => repo.GetItemsAsync())
                .ReturnsAsync(expectedItemList);

            // Act
            var result = (await controller.GetItemsAsync()).ToList();

            // Assert
            result.Should().BeOfType<List<ItemDto>>();

            result.Should().BeEquivalentTo(expectedItemList);
        }

        [Fact]
        public async Task GetItemsAsync_WithMatchingItems_ReturnsLMatchingItems()
        {
            // Arrange
            var allItems = new[] {
            ...
            foundItems.Should().OnlyContain(
                item => item.Name == allItems[0].Name
                    ...
                    || item.Name == allItems[3].Name
            );
        }

        [Fact]
        public async Task CreateItemAsync_WithValidInputs_ReturnsCreatedItem()
        {
            // Arrange
